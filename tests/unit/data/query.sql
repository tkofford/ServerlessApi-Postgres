"select d.PER_NAME_MONTH
               ...
               ,case when cvms.eff_apprvd_quote_date_wid = -1 then NULL else to_date(cvms.eff_apprvd_quote_date_wid,''yyyymmdd'') end eff_apprvd_quote_date
               ,case when cvms.eff_actvtn_quote_date_wid = -1 then NULL else to_date(cvms.eff_actvtn_quote_date_wid,''yyyymmdd'') end eff_actvtn_quote_date
               ,case when cvms.delivery_quote_date_wid = -1 then NULL else to_date(cvms.delivery_quote_date_wid,''yyyymmdd'') end delivery_quote_date
          ...
          from edw.cust_veh_month_snp_f cvms
          join edw.cust_veh_month_snp_d cvmsd on (cvms.monthly_snapshot_date_wid=cvmsd.monthly_snapshot_date_wid and cvms.vehicle_wid=cvmsd.vehicle_wid and cvms.quote_wid=cvmsd.quote_wid)
          join edw.date_d d on (cvms.monthly_snapshot_date_wid=d.row_wid)
          join edw.as_is_customer_dh_mv cdh on (cvms.customer_wid=cdh.customer_wid)
          join edw.customer_d c on (cvms.customer_wid=c.customer_wid)
          join edw.quote_d q on (cvms.quote_wid=q.quote_wid)
          join edw.as_is_quote_d_mv aiq on (cvms.quote_wid=aiq.quote_wid)
          join edw.vehicle_d v on (cvms.vehicle_wid=v.vehicle_wid)
          join edw.vehicle_class_d vc on (cvms.vehicle_class_wid=vc.vehicle_class_wid)
          join edw.quote_status_d qs on (cvms.quote_status_wid=qs.quote_status_wid)
          join edw.lease_type_d lt on (cvms.lease_type_wid=lt.lease_type_wid)
          join edw.employee_d ae on (cvms.ae_employee_wid=ae.employee_wid)
          join edw.employee_d am on (cvms.am_employee_wid=am.employee_wid)
          join edw.org_d o on (cvms.org_wid=o.org_wid)
         where 1=1
